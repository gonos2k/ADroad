...
- [x] **storage-active 검증**(`--k0 3800 --tag storage_active`, lead가 Ice@4312 포함): storage 활성 신호는 **window diag_rate 4.17%**(residual은 2.2e-16으로 **P0 tolerance(1e-9) 이내 = clean**; residual은 code-leak detector라 "≠0"으로 표현하지 않음). DA가 RMSE 1.3441→1.1866(Δ−0.1575, PASS) 개선하면서 DA/no-DA 물리 부담 동일(physics_worse=False). k0=2000(완전 비활성, diag_rate 0)과 대조 → deviation 감사가 실제로 exercised됨을 확인. artifact: `reports/forecast_da_fullmodel_storage_active.*`. (gate가 위반을 FAIL로 잡는 경로는 test_skill_gate에서 unit 검증.)
...
- [ ] A1(evolved-end correction) 비교군 구현.
